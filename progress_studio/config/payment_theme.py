...
def load_payment_line_theme(path: Path | None = None) -> PaymentLineTheme:
    ...
    return PaymentLineTheme(
        ...
        label=PaymentLabelTheme(
            width_px=max(int(label.get("width_px", 290)), 40),
            height_px=max(int(label.get("height_px", 52)), 14),
            font_size=max(int(label.get("font_size", 24)), 6),
            corner_radius_px=max(int(label.get("corner_radius_px", 12)), 0),
