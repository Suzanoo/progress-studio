...
class EarnedValueDeriver:
    """Derive EV-1 values from MainDataset + BOQ mapping provenance.

    EV-0 contract:
    - BAC is mapped BOQ amount.
    - Main period values are weekly increments; cumulative plan/actual is derived here.
    - PV = BAC * cumulative plan progress.
    - EV = BAC * cumulative actual progress.
    - EV stops after cutoff; PV may continue through project finish.
    - Every positive-amount BOQ item must be allocated to exactly 100% (within tolerance).
    - No workbook I/O or presentation rendering occurs in this service.
    """
    ...
    def derive(
        self,
        dataset: MainDataset,
        boq_rows: tuple[BOQRow, ...] | list[BOQRow],
        allocations: tuple[AllocationRecord, ...] | list[AllocationRecord],
        *,
        ...
        for record in allocations:
            if record.boq_key not in boq_by_key:
                raise EarnedValueInputError(
                    f"Allocation references unknown BOQ item: {record.boq_key}"
                )
            if record.activity_id.strip() not in plan_by_id:
                raise EarnedValueInputError(
                    "Allocation references unknown Activity ID: "
                    f"{record.activity_id} (BOQ {record.boq_key})"
                )
            ...
            for index, period in enumerate(reporting_periods):
                ...
                points.append(
                    _point(
                        period_key=period.key,
                        reporting_date=period.reporting_date,
                        pv=pv_total if pv_has_value else None,
                        ev=ev_total if ev_has_value else None,
                    )
                )
            boq_items.append(
                BOQEarnedValue(
                    boq_key=boq.key,
                    stable_id=boq.stable_id,
    ...
    def _validate_full_allocation(
        self,
        boq_rows: tuple[BOQRow, ...],
        allocation_by_boq: dict[str, list[AllocationRecord]],
    ) -> None:
        incomplete: list[str] = []
        for boq in boq_rows:
            if float(boq.amount or 0.0) <= 0.0:
                continue
            total_share = sum(
                float(record.share_percent)
                for record in allocation_by_boq.get(boq.key, [])
            )
            if not isclose(
                total_share,
                100.0,
                rel_tol=0.0,
                abs_tol=self._allocation_tolerance,
            ):
        ...
        if incomplete:
            detail = "\n".join(incomplete)
            raise EarnedValueInputError(
                "Earned Value requires every positive-amount BOQ item to be "
                f"allocated to 100%.\n{detail}"
            )
        ...
        for plan in plan_rows:
            ...
            for period in periods:
                ...
                if a_week is not None:
                    ...
                    actual_started = True
                actual_values.append(actual_acc if actual_started else None)

            result[_identity(plan)] = (plan_values, actual_values)
        return result

    @staticmethod
    def _aggregate_activity_points(
        periods: tuple,
        activities: tuple[ActivityEarnedValue, ...],
    ) -> tuple[EarnedValuePoint, ...]:
        points: list[EarnedValuePoint] = []
